...
@comp score(params::CtcOutput, xs::Vector) = [score(params, x) for x in xs]

@comp probs(params::CtcOutput, xs::Vector) = [softmax(y) for y in score(params, xs)]

@comp predict(params::CtcOutput, xs::Vector) = [argmax(y) for y in score(params, xs)]

@comp function cost(params::CtcOutput, xs::Vector, ys::Vector)
