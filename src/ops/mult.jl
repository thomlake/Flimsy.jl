...
Gradient propagation
    ...
    db[i] += dc[i] * a[i]
"""
type ReverseMult{A<:AbstractValue,B<:AbstractValue} <: ReverseOperation
    c::Variable
    a::A
    b::B
end
...
for (A, B) in [(Constant,Variable), (Variable,Constant), (Variable,Variable)]
    updates = Any[]
    A <: Variable && push!(updates, :(a.grad[i] += c.grad[i] * b.data[i]))
    B <: Variable && push!(updates, :(b.grad[i] += c.grad[i] * a.data[i]))
    update_block = Expr(:block, updates...)
    defn = :(function call(rop::ReverseMult{$A,$B})
        c = rop.c
        a = rop.a
        b = rop.b
...
mult_elementwise(a::AbstractArray, b::AbstractArray) = mult_elementwise!(similar(a), a, b)

mult(scope::Scope, a::AbstractValue, b::AbstractValue) = Constant(mult_elementwise(a.data, b.data))

for (A, B) in [(Constant,Variable), (Variable,Constant), (Variable,Variable)]
    defn = :(function mult(scope::GradScope, a::$A, b::$B)
        c = Variable(mult_elementwise(a.data, b.data))
